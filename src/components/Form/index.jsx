...
const Form = ({ signIn, signUp }) => {
	return (
		<FormStyle signIn={signIn}>
			<Card signIn={signIn}>
				<header>{signIn ? <>{'Welcome'} &#129309;&#127995;</> : "Create Account with Us"} </header>
				<main>
					...
					<div className="news_letter">
						<div className="input_wrapper">
							...
							<label htmlFor="female">{"I want to receive newsletter"}</label>
						</div>
					</div>
					</>)}
				</main>
				...
				<footer>
					<Button style={{ width: "100%" }} primary text={signIn? "Sign In" : "Create Account"} />
				</footer>
			</Card>
		</FormStyle>
	);
